...
'''class TestQuestion(APITestCase):
    ...
    def test_create(self):
        ...
        response = self.client.post(reverse('question'), data={'label': 'Quel est ton prénom ?'})
        self.assertEqual(response.status_code, 200)
        question = Question.objects.get()
        self.assertEqual(response.json(), {'id': question.pk, 'label': question.label})
'''


class TestCategory_detail(ShopAPITestCase):
